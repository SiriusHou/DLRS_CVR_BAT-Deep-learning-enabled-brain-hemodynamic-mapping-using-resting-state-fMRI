function cvr_t1roi_cvr_average(path_mpr,name_mpr,acbffile,rcbffile,rmskfile,outpath)
...
% P0 = spm_select('FPList',path_mpr,['^' name_mpr '.*' atlas_ver '.*MNI_2mm.img$']);
roimaskfile = P0(1,:);
...
% msk1    = ~(acbf==0); % use cvr map, not brainmask as brainmsk

% Combine parcellations to Type1-L2 or Type1-L3 segmentations
% [~,cbffname,~] = fileparts(acbffile);
% fresult  = [outpath filesep cbffname(1:end-14) '_CVR_T1segmented_ROIs.txt'];
fresult  = [outpath filesep 'cvr_roi_results_mskonrmap.txt'];
% fresult  = [outpath filesep 'cvr_roi_results_glovoxshft.txt'];
fid        = fopen(fresult, 'wt');
coltitle = 'ROI analysis by %d major tissue types\n';
colnames = 'Index\tMask_name\tNumber_of_voxels\tcvr_gloshft\tcvr_voxshft\n';
colformt = '%d\t%s\t%u\t%1.4f\t%1.4f\n';
...
%     spm_write_vol(outVol, segmask);
    ...
    for ii = 1:seg_num
        ...
        seg_name = roi_lst{ii};
        fprintf(fid,colformt,seg_idx(ii),seg_name,seg_nvox,seg_acbf,seg_rcbf);
    end
    fprintf(fid,'\n\n');
end
...
%     fprintf(fid,colformt,label_idx(ii),seg_name,seg_nvox,seg_acbf,seg_rcbf);
% end
