function zach_T1ROI_CVRaverage(path_mpr,name_mpr,acvrfile,rcvrfile,rmskfile,outpath)
% calculate the CVR ROI average for three segmentation levels (Type1-L5, Type1-L3, and Type1-L2)
% yli20160627
...
acvr    = spm_read_vols(V1); acvr(isnan(acvr)) = 0;
rcvr    = spm_read_vols(V2); rcvr(isnan(rcvr)) = 0;
msk1    = spm_read_vols(V3); msk1(isnan(msk1)) = 0;


% Combine parcellations to Type1-L2 or Type1-L3 segmentations
[~,cvrfname,~] = fileparts(acvrfile);
fresult  = [outpath filesep cvrfname(1:end-14) '_CVR_T1segmented_ROIs.txt'];
fid        = fopen(fresult, 'wt');
coltitle = 'ROI analysis by %d major tissue types\n';
colnames = 'Index\tMask_name\tRegional_CVR(deltaBOLD/mmHg EtCo2)\tRegional_relative_CVR\tNumber_of_voxels\n';
...
for tt = 1:length(roi_lists_info)-1
    ...
    % calculate the ROI CVR average and print to text file
    ...
    for ii = 1:seg_num
        ...
        seg_acvr = mean( acvr( idxvox_seg ) );
        seg_rcvr = mean( rcvr( idxvox_seg ) );
        seg_nvox = length( acvr( idxvox_seg ) );
        seg_name = roi_lst{ii};
        fprintf(fid,colformt,seg_idx(ii),seg_name,seg_acvr,seg_rcvr,seg_nvox);
    end
    fprintf(fid,'\n\n');
end

% calculate the ROI CVR average of Type1-L5 segmentations and print
...
for ii = 1:label_num
    ...
    fprintf(fid,colformt,label_idx(ii),seg_name,seg_acvr,seg_rcvr,seg_nvox);
end
