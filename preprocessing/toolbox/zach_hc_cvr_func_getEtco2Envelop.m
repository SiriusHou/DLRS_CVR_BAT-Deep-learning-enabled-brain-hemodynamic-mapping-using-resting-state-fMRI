function [txtname_etco2] = zach_hc_cvr_func_getEtco2Envelop(physiodir, txtname_co2_raw, time_bold_start, bold_scandur, select)
...
timer_sync_mri      = time2num(time_bold_start); % scanner recording
time_add_ahead      = time2num('00:00:45'); % add 45 sec ahead
% time_add_behind     = time2num('00:00:30'); % add 30 sec behind (unused)

% other constant
scan_duration       = floor( bold_scandur ); %+ time_add_ahead + 
    % time_add_behind; % add 45 sec before (x->) and 30 sec after (<-x) mri scan
sample_delay_co2    = +6;  % tubing delay +6 sec (philips monitor)
sample_rate_co2     = 100; % sampling rate for etco2 100 Hz (philips monitor)
...
% subject mouth as real etco2 recording, set bold start time as 0, and consider tubing delay
timep_etco2 = ( 1:length(vec_co2) )' / sample_rate_co2 - timer_sync_mri - sample_delay_co2;
...
% generate interploation timestamps (frequency of 1 Hz)
interpTime = latestStartTime:earliestEndTime;
...
dlmwrite([physiodir filesep txtname_co2_raw(1:end-4) '_envelop_45sec.txt'], [timepeaks co2envlp]);


%% wrap up
txtname_etco2   = [physiodir filesep txtname_co2_raw(1:end-4) '_envelop_45sec.txt'];
